...
class SubtitleGenerator:
    ...
    def _apply_srt_formatting(self, text: str, font_config: Dict) -> str:
        ...
        # SRT suporta formatação HTML básica
        formatted = text
        
        if font_config.get('bold', False):
            formatted = f"<b>{formatted}</b>"
        
        # Nota: SRT não suporta cor e tamanho via HTML padrão
        # Alguns players aceitam tags <font>, mas não é padrão
    ...
    def _generate_srt(self, file_path: str, subtitles: List[Dict]):
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            for i, sub in enumerate(subtitles, 1):
                start = self._format_timedelta_srt(sub['start'])
                end = self._format_timedelta_srt(sub['end'])
    ...
    def _generate_ass(self, file_path: str, subtitles: List[Dict]):
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            ...
            for sub in subtitles:
                start = self._format_timedelta_ass(sub['start'])
                end = self._format_timedelta_ass(sub['end'])
                text = sub.get('formatted_text', sub.get('text', ''))
